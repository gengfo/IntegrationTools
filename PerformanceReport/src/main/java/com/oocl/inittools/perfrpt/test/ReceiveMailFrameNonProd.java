...
public class ReceiveMailFrameNonProd extends JFrame {
    private JTextArea ta_receive;
    protected Session session;
    protected Store store;
    
    
    String username = "tf04root\\gengfo";
    //final String protocol = "imaps";
    //final String server = "testhkgowa";
    final int port = 993;
    private String password = "Password2";// 密码
    
    private String receiveHost = "testhkgowa";// 接收主机
    ...
    public static void main(String args[]) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    ReceiveMailFrameNonProd frame = new ReceiveMailFrameNonProd();
                    frame.init();// 调用初始化方法
    ...
    public ReceiveMailFrameNonProd() {
        super();
    ...
    public void readMessage(Message message, IMAPFolder folder, int i)
        ...
        if (content instanceof Multipart) {// 有附件执行的代码
            ...
            try {
                ...
                FileDialog dialog = new FileDialog(ReceiveMailFrameNonProd.this, "保存");// 创建对话框
